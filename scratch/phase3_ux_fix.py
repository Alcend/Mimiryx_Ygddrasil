...
# 1. Imports
if 'import ReactMarkdown from' not in text:
    text = text.replace("import React, { useState } from 'react';", "import React, { useState } from 'react';\nimport ReactMarkdown from 'react-markdown';")
if 'BrainCircuit' not in text:
    text = text.replace("Loader2,\n} from 'lucide-react';", "Loader2,\n  BrainCircuit,\n  Type,\n} from 'lucide-react';")

# 2. Button
old_btn = re.search(r'<button[^>]*onClick=\{handleAutoOrganize\}[^>]*>.*?</button>', text, re.DOTALL)
if old_btn:
    new_btn = """                <button
    ...
    text = text.replace(old_btn.group(0), new_btn)

# 3. Editor
old_editor = re.search(r'\{\/\* Editor Body \*\/\}.*?<textarea[^>]*\/>\s*<\/div>', text, re.DOTALL)
if old_editor:
    ...
    text = text.replace(old_editor.group(0), new_editor)
